			...
			<?php
				...
				if(isset($client_key)){
					foreach(get_tags($client_key) as $tag){
						echo "<option value='" . $tag["tag_name"] . "' " . ((isset($tag_name)&&tag==$tag["tag_name"])?"selected":"") . ">" . htmlentities($tag["tag_name"]) . "</option>";
					}
				}
			?>
			</select>
...
<?php
	...
	if($args["since"] != ""){
		$stmt->bindValue(":created",$args["since"],PDO::PARAM_INT);
	}
	$stmt->execute();
	...
	if (isset($_GET["csv"])){
		...
		//data
			foreach ($rows as $row){
				...
				foreach ($row as $k => $v){	
				//hackhack ...
					if($k == "created" && isset($_GET["nicedate"]))
